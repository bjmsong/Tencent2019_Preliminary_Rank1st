...
for path1, path2, flag, wday in [('data/train_dev.pkl', 'data/dev.pkl', 'dev', 1),
                                 ('data/train.pkl', 'data/test.pkl', 'test', 3)]:
    print(path1, path2)
    ...
    for i in range(k):
        train.append((path1[:-4] + '_NN_' + str(i) + '.pkl',
                      train_df.iloc[int(i / k * len(train_df)):int((i + 1) / k * len(train_df))]))
    del train_df
    gc.collect()
