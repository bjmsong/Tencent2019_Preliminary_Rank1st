...
def kfold_static(train_df, test_df, f, label):
    ...
    num = len(train_df) // 5
    index = [0 for i in range(num)] + [1 for i in range(num)] + [2 for i in range(num)] + [3 for i in range(num)] + \
            [4 for i in range(len(train_df) - 4 * num)]
...
if __name__ == "__main__":
    for path1, path2, log_path, flag, wday, day in [
        ('../../train_dev.pkl', '../../dev.pkl', '../../user_log_dev.pkl', 'dev', 1, 17974),
        ('../../train.pkl', '../../test.pkl', '../../user_log_test.pkl', 'test', 3, 17976)]:
        # 拼接静态特征
        ...
        df = pd.read_pickle('../../testA/ad_static_feature.pkl')
