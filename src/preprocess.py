...
def parse_rawdata():
    # 曝光日志
    df = pd.read_csv('../../testA/totalExposureLog.out', sep='\t',
                     names=['id', 'request_timestamp', 'position', 'uid', 'aid', 'imp_ad_size', 'bid', 'pctr',
                            'quality_ecpm', 'totalEcpm']).sort_values(by='request_timestamp')
    df[['id', 'request_timestamp', 'position', 'uid', 'aid', 'imp_ad_size']] = df[
        ['id', 'request_timestamp', 'position', 'uid', 'aid', 'imp_ad_size']].astype(int)
    df[['bid', 'pctr', 'quality_ecpm', 'totalEcpm']] = df[['bid', 'pctr', 'quality_ecpm', 'totalEcpm']].astype(float)
    df.to_pickle('../../testA/totalExposureLog.pkl')
    # del删除的是变量，而不是数据
    del df
    # 触发垃圾回收
    gc.collect()
    ##############################################################################
    # 静态广告
    df = pd.read_csv('../../testA/ad_static_feature.out', sep='\t',
                     names=['aid', 'create_timestamp', 'advertiser', 'good_id', 'good_type', 'ad_type_id',
                            'ad_size']).sort_values(by='create_timestamp')
    df = df.fillna(-1)
    ...
    df.to_pickle('../../testA/ad_static_feature.pkl')
    del df
    gc.collect()
    ##############################################################################
    # 用户信息
    df = pd.read_csv('../../testA/user_data', sep='\t',
                     names=['uid', 'age', 'gender', 'area', 'status', 'education', 'concuptionAbility', 'os', 'work',
    ...
    df.to_pickle('../../testA/user_data.pkl')
    del df
    gc.collect()
    ##############################################################################
    # 测试数据
    df = pd.read_csv('../../testA/test_sample.dat', sep='\t',
                     names=['id', 'aid', 'create_timestamp', 'ad_size', 'ad_type_id', 'good_type', 'good_id',
                            'advertiser', 'delivery_periods', 'crowd_direction', 'bid'])
    df = df.fillna(-1)
    ...
    df.to_pickle('../../testA/test_sample.pkl')
    del df
    gc.collect()
...
def construct_log():
    ...
    train_df = pd.read_pickle('../../testA/totalExposureLog.pkl')
    ...
    dev_df = train_df[train_df['request_day'] == 17974]  # 验证集：2019.3.19
    # drop maybe better
    del dev_df['period_id']
    del dev_df['minute']
    del dev_df['hour']
    log = train_df
    tmp = pd.DataFrame(train_df.groupby(['aid', 'request_day']).size()).reset_index()  # 计算广告的日曝光
...
def extract_setting():
    aids = []
    with open('../../testA/ad_operation.dat', 'r') as f:
        for line in f:
...
def construct_train_data(train_df):
    # 构造训练集
    # 算出广告当天平均出价和曝光量
    tmp = pd.DataFrame(train_df.groupby(['aid', 'request_day'])[
                           'bid'].nunique()).reset_index()  # nunique: Count distinct observations over requested axis
    ...
    train_df = train_df.drop_duplicates(
        ['aid', 'request_day'])  # Only consider certain columns for identifying duplicates
    train_df = train_df.merge(tmp, on=['aid', 'request_day'], how='left')
...
test_df = pd.read_pickle('../../testA/test_sample.pkl')

print("combine advertise features ....")
ad_df = pd.read_pickle('../../testA/ad_static_feature.pkl')
...
train_dev_df.to_pickle('../../train_dev.pkl')
train_df.to_pickle('../../train.pkl')
dev_df.to_pickle('../../dev.pkl')
